...
export class AgentLanguageInterface {
    ...
    try {
      ...
      if (this.openAIAssistant) {
        ...
        if (this.openAIAssistant.updateTextEvent && this.openAIAssistant.updateTextEvent.add) {
          ...
          print("AgentLanguageInterface: ⚠️ OpenAI updateTextEvent not available yet");
        }
        ...
        if (this.openAIAssistant.functionCallEvent && this.openAIAssistant.functionCallEvent.add) {
          ...
          print("AgentLanguageInterface: ⚠️ OpenAI functionCallEvent not available yet");
        }
      }
      ...
      if (this.geminiAssistant) {
        if (this.geminiAssistant.updateTextEvent && this.geminiAssistant.updateTextEvent.add) {
          ...
          print("AgentLanguageInterface: ⚠️ Gemini updateTextEvent not available yet");
        }
        ...
        if (this.geminiAssistant.functionCallEvent && this.geminiAssistant.functionCallEvent.add) {
          ...
          print("AgentLanguageInterface: ⚠️ Gemini functionCallEvent not available yet");
        }
      }
  ...
  public switchProvider(provider: 'openai' | 'gemini'): void {
    if (provider === this.currentProvider) {
      print(`AgentLanguageInterface: ℹ️ Already using ${provider} provider`);
      return;
    }
  ...
  private initializeCurrentProvider(): void {
    if (this.providerInitialized[this.currentProvider]) {
      print(`AgentLanguageInterface: ℹ️ ${this.currentProvider} session already initialized`);
      return;
    }
    ...
    try {
      if (this.currentProvider.toLowerCase() === 'openai' && this.openAIAssistant) {
        ...
        print(`AgentLanguageInterface: ❌ Debug - OpenAI check: ${this.currentProvider.toLowerCase() === 'openai'} && ${!!this.openAIAssistant}`);
        print(`AgentLanguageInterface: ❌ Debug - Gemini check: ${this.currentProvider.toLowerCase() === 'gemini'} && ${!!this.geminiAssistant}`);
  ...
  public async generateResponse(messages: Message[], options?: LLMOptions): Promise<LLMResponse> {
    ...
    if (providerToUse === 'gemini' && !this.geminiAssistant && this.openAIAssistant) {
      print("AgentLanguageInterface: ⚠️ Gemini not available - falling back to OpenAI");
      ...
      print("AgentLanguageInterface: ⚠️ OpenAI not available - falling back to Gemini");
      providerToUse = 'gemini';
    }
    ...
    try {
      print(`AgentLanguageInterface: 🎯 Using provider: ${providerToUse} for response generation (voice: ${!options?.textOnly})`);
  ...
  public async generateTextResponse(messages: Message[], options?: LLMOptions): Promise<string> {
    ...
    if (!this.providerInitialized[this.currentProvider]) {
      print(`AgentLanguageInterface: ⚠️ ${this.currentProvider} session not initialized, attempting to initialize...`);
      this.initializeCurrentProvider();
  ...
  private generateFallbackResponse(messages: Message[]): LLMResponse {
    ...
    print("AgentLanguageInterface: ⚠️ Using fallback response due to AI backend unavailability");
  ...
  private async generateGeminiResponse(messages: Message[], options?: LLMOptions): Promise<LLMResponse> {
    if (!this.geminiAssistant) {
      print("AgentLanguageInterface: ⚠️ Gemini Assistant not available, using fallback");
      return this.generateFallbackResponse(messages);
    }
    ...
    try {
      ...
      print(`AgentLanguageInterface: ✅ Gemini Live response received: "${responseText.substring(0, 100)}..." (${responseText.length} chars)`);
    ...
    return new Promise<string>((resolve) => {
      ...
      const completeResponse = (reason: string) => {
        if (!responseComplete) {
          ...
          print(`AgentLanguageInterface: ✅ Text collection completed (${reason}): "${responseText.substring(0, 100)}..." (${responseText.length} chars)`);
  ...
  private sendMultimodalMessageToGemini(textContent: string, imageData: string): void {
    if (!this.geminiAssistant) {
      print("AgentLanguageInterface: ⚠️ Gemini Assistant not available for multimodal message");
      return;
    }
  ...
  public streamData(stream: boolean): void {
    if (!this.isInitialized) {
      print("AgentLanguageInterface: ⚠️ Session not initialized, cannot stream data");
      return;
    }
    ...
    try {
      if (this.currentProvider.toLowerCase() === 'openai' && this.openAIAssistant) {
        ...
        print(`AgentLanguageInterface: ℹ️ OpenAI streaming ${stream ? 'started' : 'stopped'}`);
      } else if (this.currentProvider.toLowerCase() === 'gemini' && this.geminiAssistant) {
        this.geminiAssistant.streamData(stream);
        print(`AgentLanguageInterface: ℹ️ Gemini streaming ${stream ? 'started' : 'stopped'}`);
      }
    } catch (error) {
  ...
  public interruptAudioOutput(): void {
    try {
      if (this.currentProvider.toLowerCase() === 'openai' && this.openAIAssistant) {
        ...
        print("AgentLanguageInterface: ℹ️ OpenAI audio interrupted");
      } else if (this.currentProvider.toLowerCase() === 'gemini' && this.geminiAssistant) {
        this.geminiAssistant.interruptAudioOutput();
        print("AgentLanguageInterface: ℹ️ Gemini audio interrupted");
      }
    } catch (error) {
    ...
    try {
      if (this.currentProvider.toLowerCase() === 'openai' && this.openAIAssistant) {
        if (callId) {
          ...
          print("AgentLanguageInterface: ⚠️ OpenAI requires callId for function results");
        }
      } else if (this.currentProvider.toLowerCase() === 'gemini' && this.geminiAssistant) {
  ...
  private handleTextUpdate(data: { text: string; completed: boolean }, provider: string): void {
    ...
    if (data.text && data.text.length > 0) {
      print(`AgentLanguageInterface: 📝 Text update from ${provider}: "${data.text.substring(0, 50)}..." (completed: ${data.completed})`);
    }
  }
